...
- By default, log autonomous method or code changes in [`docs/autonomous_notes.md`](docs/autonomous_notes.md) or another non-paper note in `docs/`, not in `paper/`.
- The active manuscript location is `paper/`. Do not treat manuscript-like files in the parent repo, such as [`docs/paper.tex`](docs/paper.tex), as the default paper editing target unless the user explicitly asks for those legacy files.
...
For implementation, methods, experiments, and autonomous findings, update code and non-paper docs first, such as [`docs/autonomous_notes.md`](docs/autonomous_notes.md), unless the user explicitly asks for manuscript edits.
...
- [`docs/autonomous_notes.md`](docs/autonomous_notes.md) is the default running log for those notes when no more specific doc is requested.
