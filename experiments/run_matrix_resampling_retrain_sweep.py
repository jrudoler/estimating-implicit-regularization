#!/usr/bin/env python3
"""Small sweep for retrain-per-resample matrix resampling variants."""
...
RETRAIN_RE = re.compile(
    r"RetrainResample \| pair=(?P<pair>[a-z_]+__[a-z_]+) mode=(?P<mode>[a-z_]+) "
    r"gradient_dataset=(?P<gradient_dataset>[a-z_]+) "
    r"full_cos=(?P<full_cos>[-+0-9.eE]+) full_cond=(?P<full_cond>[-+0-9.eE]+) "
    r"full_ols_mean_rel_error=(?P<full_ols_mean_rel_error>[-+0-9.eE]+) "
    ...
    r"replicate_cos_std=(?P<replicate_cos_std>[-+0-9.eE]+) "
    r"replicate_cond_mean=(?P<replicate_cond_mean>[-+0-9.eE]+) "
    r"full_train_mse=(?P<full_train_mse>[-+0-9.eE]+) full_test_mse=(?P<full_test_mse>[-+0-9.eE]+) "
    r"replicate_train_mse_mean=(?P<replicate_train_mse_mean>[-+0-9.eE]+) "
    r"replicate_test_mse_mean=(?P<replicate_test_mse_mean>[-+0-9.eE]+)"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("logs/phase2/matrix_resampling_retrain_sweep.csv"),
    )
    return parser.parse_args()


def summarize_teacher_spectrum(output_dim: int, input_dim: int, teacher_rank: int, decay: float) -> dict[str, float]:
...
def parse_run_output(output_text: str) -> dict[str, str]:
    match = RETRAIN_RE.search(output_text)
    if not match:
        raise RuntimeError("Failed to parse RetrainResample line from run output.")
