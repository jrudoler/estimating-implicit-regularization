#!/usr/bin/env python3
"""Retrain-per-resample study for shared-lambda recovery in the matrix-spectrum control."""
...
class RetrainResamplingSummary:
...
def train_model(
    ...
    set_seed(seed)
    batch_size = min(len(train_dataset), 4096)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0)
    val_loader = DataLoader(val_dataset, batch_size=min(len(val_dataset), 4096), shuffle=False, num_workers=0)
...
def run_retrain_resampling_study(
    train_dataset: TensorDataset,
    val_dataset: TensorDataset,
    ...
    full_model = train_model(
        ...
        lr=lr,
        optimizer_name=optimizer_name,
        explicit_bias_types=bias_names,
        explicit_lambdas=explicit_lambdas,
        max_epochs=max_epochs,
        patience=patience,
        accelerator=accelerator,
    ...
    return RetrainResamplingSummary(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--teacher-spectrum-decay", type=float, default=0.35)
...
def main() -> None:
    args = parse_args()
    run = wandb.init(project=args.wandb_project, job_type="matrix_spectrum_resampling_retrain")
    cfg = run.config
    ...
    if len(gt_bias_types) != 2:
        raise ValueError("This retrain resampling study expects exactly two bias types.")
    ...
    if auto_balance_gt_lambdas:
        reference_model = LinearMatrixRegressor(
            input_dim=input_dim,
            output_dim=output_dim,
            lr=lr,
            optimizer_name=optimizer_name,
        ...
        gt_lambdas = calibrate_explicit_lambdas(reference_model.network, gt_bias_types, gt_lambda_scale)

    gt_map = {name: value for name, value in zip(gt_bias_types, gt_lambdas)}
    summary = run_retrain_resampling_study(
        train_dataset=train_dataset,
        val_dataset=val_dataset,
    ...
    scalar_logs = {
        "fit/full_train_mse": summary.full_train_mse,
        "fit/full_test_mse": summary.full_test_mse,
        "fit/replicate_train_mse_mean": summary.replicate_train_mse_mean,
        "fit/replicate_test_mse_mean": summary.replicate_test_mse_mean,
    ...
    LOGGER.info(
        ...
        summary.replicate_design_condition_mean,
        summary.full_train_mse,
        summary.full_test_mse,
        summary.replicate_train_mse_mean,
        summary.replicate_test_mse_mean,
    )
    for bias_name, true_value in gt_map.items():
        LOGGER.info("  %s true=%.6f", bias_name, true_value)
