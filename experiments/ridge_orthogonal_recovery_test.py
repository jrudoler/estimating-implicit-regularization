#!/usr/bin/env python3
"""
Ridge + Orthogonal recovery test with normalized gradient matching.
Uses MSE loss for regression.
Orthogonal regularizer showed stronger signal in screening test than Nuclear norm.
"""
...
class BiasWithMSENormalized(BiasWithMSE):
    ...
    def _compute_penalty_gradient(self, bias_model: nn.Module) -> Tuple[torch.Tensor, torch.Tensor]:
        flat_params = (
            ...
            .detach().requires_grad_().to(self.device)
        )
...
class BiasWithMSEScheduled(BiasWithMSE):
    ...
    def __init__(self, *args, bias_lr: float = 0.1, patience_lr: int = 10, 
...
def compute_orthogonal_penalty(model):
    """Compute orthogonal regularizer: sum of ||W^T W - I||_F^2 for each layer."""
    penalty = 0.0
    for p in model.parameters():
        if p.ndim >= 2:
            W = p if p.ndim == 2 else p.reshape(p.shape[0], -1)
            WTW = W.T @ W
            eye = torch.eye(WTW.shape[0], device=W.device)
            penalty += (WTW - eye).pow(2).sum()
    return penalty


def train_network(gt_ridge, gt_orthogonal, loader, device, seed=42):
    ...
    for epoch in range(100):
        for X_b, y_b in loader:
            ...
            if gt_orthogonal > 0:
                orth = compute_orthogonal_penalty(net)
                loss = loss + gt_orthogonal * orth
...
def compute_geometry(net, X, y, device):
    ...
    orthb = OrthogonalBias(enforce_positive=True)
    orthb.beta.data = torch.tensor(0.0, device=device)
    orthb = orthb.to(device)
    orth_val = orthb(flat2, struct2)
    grad_orth = torch.autograd.grad(orth_val, flat2)[0]
    
    cos_r = F.cosine_similarity(target.unsqueeze(0), grad_r.unsqueeze(0)).item()
    cos_orth = F.cosine_similarity(target.unsqueeze(0), grad_orth.unsqueeze(0)).item()
    cos_r_orth = F.cosine_similarity(grad_r.unsqueeze(0), grad_orth.unsqueeze(0)).item()
    
    return {
        'target_norm': target.norm().item(),
        'grad_r_norm': grad_r.norm().item(),
        'grad_orth_norm': grad_orth.norm().item(),
        'cos_target_r': cos_r,
        'cos_target_orth': cos_orth,
        'cos_r_orth': cos_r_orth,
    }
...
def estimate_bias(net, loader, method='standard', device='cpu'):
    ...
    orth = OrthogonalBias(enforce_positive=True, init_value=0.01)
    joint = JointBias([ridge, orth])
    ...
    if method == 'standard':
        ...
        return params['ridge/scale'], params['orthogonal/scale']
    else:
        lambdas = estimator.get_estimated_lambdas()
        return lambdas['ridge'], lambdas['orthogonal']
...
def main():
    ...
    # Ground truth combinations - using smaller values for orthogonal
    # since it has a different scale
    ridge_values = [0.0, 0.001, 0.01]
    orthogonal_values = [0.0, 0.0001, 0.001]
    
    results = []
    
    for gt_ridge, gt_orthogonal in product(ridge_values, orthogonal_values):
        print(f"\n{'='*60}")
        print(f"Ground truth: Ridge={gt_ridge}, Orthogonal={gt_orthogonal}")
        print('='*60)
        
        net = train_network(gt_ridge, gt_orthogonal, loader, device)
        
        geom = compute_geometry(net, X[:256], y[:256], device)
        print(f"Geometry: cos(t,R)={geom['cos_target_r']:.3f}, cos(t,O)={geom['cos_target_orth']:.3f}, cos(R,O)={geom['cos_r_orth']:.3f}")
        print(f"          ||target||={geom['target_norm']:.4f}, ||∇R||={geom['grad_r_norm']:.2f}, ||∇O||={geom['grad_orth_norm']:.2f}")
        
        est_r_std, est_o_std = estimate_bias(net, loader, method='standard', device=device)
        r_err_std = abs(est_r_std - gt_ridge) / max(gt_ridge, 0.0001) * 100 if gt_ridge > 0 else abs(est_r_std) * 1000
        o_err_std = abs(est_o_std - gt_orthogonal) / max(gt_orthogonal, 0.00001) * 100 if gt_orthogonal > 0 else abs(est_o_std) * 10000
        print(f"Standard:   Ridge={est_r_std:.6f} (err {r_err_std:.0f}%), Orthogonal={est_o_std:.6f} (err {o_err_std:.0f}%)")
        
        est_r_norm, est_o_norm = estimate_bias(net, loader, method='normalized', device=device)
        r_err_norm = abs(est_r_norm - gt_ridge) / max(gt_ridge, 0.0001) * 100 if gt_ridge > 0 else abs(est_r_norm) * 1000
        o_err_norm = abs(est_o_norm - gt_orthogonal) / max(gt_orthogonal, 0.00001) * 100 if gt_orthogonal > 0 else abs(est_o_norm) * 10000
        print(f"Normalized: Ridge={est_r_norm:.6f} (err {r_err_norm:.0f}%), Orthogonal={est_o_norm:.6f} (err {o_err_norm:.0f}%)")
        
        results.append({
            'gt_ridge': gt_ridge,
            'gt_orthogonal': gt_orthogonal,
            **geom,
            'std_ridge': est_r_std,
            'std_orthogonal': est_o_std,
            'std_ridge_err': r_err_std,
            'std_orthogonal_err': o_err_std,
            'norm_ridge': est_r_norm,
            'norm_orthogonal': est_o_norm,
            'norm_ridge_err': r_err_norm,
            'norm_orthogonal_err': o_err_norm,
        })
    ...
    inactive_orth = df[df['gt_orthogonal'] == 0]
    ...
    print(f"  Orthogonal inactive: Standard={inactive_orth['std_orthogonal'].abs().mean():.6f}, Normalized={inactive_orth['norm_orthogonal'].abs().mean():.6f}")
    
    active_ridge = df[df['gt_ridge'] > 0]
    active_orth = df[df['gt_orthogonal'] > 0]
    ...
    print(f"  Orthogonal active: Standard={active_orth['std_orthogonal_err'].mean():.0f}%, Normalized={active_orth['norm_orthogonal_err'].mean():.0f}%")
    
    both_active = df[(df['gt_ridge'] > 0) & (df['gt_orthogonal'] > 0)]
    if len(both_active) > 0:
        ...
        print(f"  Orthogonal: Standard={both_active['std_orthogonal_err'].mean():.0f}%, Normalized={both_active['norm_orthogonal_err'].mean():.0f}%")
    
    reg_active = df[(df['gt_ridge'] > 0) | (df['gt_orthogonal'] > 0)]
    ...
    print(f"  avg cos(t,O):      {reg_active['cos_target_orth'].mean():.3f}")
    print(f"  avg cos(R,O):      {reg_active['cos_r_orth'].mean():.3f}")
    
    df.to_csv('ridge_orthogonal_recovery_results.csv', index=False)
    print("\nResults saved to ridge_orthogonal_recovery_results.csv")
